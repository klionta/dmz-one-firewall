...
iptables -A FORWARD  -p tcp --dport 80  -d 172.18.0.2 -m conntrack --ctstate NEW -j ACCEPT

# Allow only Postgress connections in the direction dmz -> internal (private LAN)
iptables -A FORWARD  -p tcp --dport 5432 -s 172.18.0.2 -d 172.19.0.2 -m conntrack --ctstate NEW -j ACCEPT

# Allow dmz and internal to initiate connections with the outside network
iptables -A FORWARD -s 172.18.0.2 -m conntrack --ctstate NEW -j ACCEPT
iptables -A FORWARD -s 172.19.0.2 -m conntrack --ctstate NEW -j ACCEPT
